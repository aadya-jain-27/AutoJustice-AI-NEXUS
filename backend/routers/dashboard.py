...
def get_dashboard_stats(db: Session = Depends(get_db)):
    ...
    firs_gen = db.query(func.count(Report.id)).filter(Report.status == "FIR_GENERATED").scalar() or 0
...
def forensics_summary(db: Session = Depends(get_db)):
...
def explain_ai_decision(report_id: str, db: Session = Depends(get_db)):
    """
...
def live_stats(db: Session = Depends(get_db)):
    ...
    """
    total = db.query(func.count(Report.id)).scalar() or 0
    firs  = db.query(func.count(Report.id)).filter(Report.status == "FIR_GENERATED").scalar() or 0
...
def reporter_trust_summary(db: Session = Depends(get_db)):
