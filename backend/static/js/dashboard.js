...
async function loadFIRRegistry() {
  try {
    const res  = await fetch('/api/reports/?status=COMPLAINT_REGISTERED&limit=200');
    const data = await res.json();
...
async function loadForensicsSummary() {
  ...
  try {
    const [fRes, rRes] = await Promise.all([
      fetch('/api/dashboard/forensics-summary'),
...
async function loadTrustSummary() {
  ...
  if (!container) return;
  try {
    const res = await fetch('/api/dashboard/reporter-trust-summary');
    const data = await res.json();
    container.innerHTML = `
      <div class="stat-grid" style="margin-bottom:24px">
...
async function loadAuditLog() {
  ...
  try {
    const res  = await fetch('/api/dashboard/audit-log?limit=50');
...
async function showAIExplain(reportId, caseNumber) {
  ...
  try {
    const res  = await fetch(`/api/dashboard/explain/${reportId}`);
    const data = await res.json();
