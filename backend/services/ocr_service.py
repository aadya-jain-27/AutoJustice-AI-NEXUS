...
# Uses google-generativeai (stable SDK, separate from google-genai used elsewhere)
try:
    import google.generativeai as _genai_vision
...
class OCRService:
    ...
    def _extract_via_gemini_vision(self, file_path: Path) -> Tuple[str, float]:
        ...
        Uses google-generativeai (gemini-1.5-flash — fast and cheap for OCR tasks).
        ...
        try:
            ...
            _genai_vision.configure(api_key=api_key)
            model = _genai_vision.GenerativeModel("gemini-1.5-flash")

            # Upload image bytes for vision analysis
            ...
            logger.info(
                f"Gemini Vision OCR: {len(cleaned)} chars from {file_path.name}"
            )
