...
class AITriageService:
    ...
    def _fallback_analyze(self, description: str, evidence_text: str) -> TriageResult:
        ...
        high_indicators = {
            ...
            "suicide": 3, "bomb": 5, "kill": 3, "fake profile": 3,
        }
        medium_indicators = {
            "harassment": 2, "abuse": 2, "fraud": 2, "scam": 2, "phishing": 2,
            "impersonation": 2, "stalking": 2, "cheat": 2,
            # Common cybercrime patterns that alone signal MEDIUM risk
        ...
        # Any report that names a specific monetary loss is at least MEDIUM risk.
        # Covers: "45,000 rs" / "Rs. 45,000" / "INR 45000" / "₹45000" / "45000 rupees"
        ...
        if _amount_re.search(text):
            medium_score += 3   # Specific financial loss → guaranteed MEDIUM minimum
        ...
        categories = {
            "Financial Crime": ["upi", "bank", "money", "transfer", "payment", "fraud"],
        ...
        return TriageResult(
            ...
            crime_subcategory="General",
            ai_summary=f"Automated rule-based triage: {risk_level} risk {detected_category} incident. Manual review recommended.",
