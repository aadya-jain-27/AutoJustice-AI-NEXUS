...
class FakeDetectionService:
    ...
    def __init__(self):
        ...
        self._submitted_hashes: set = set()  # In-memory duplicate store (use Redis in prod)
    ...
    def analyze(
        ...
        l5_score, l5_flags = self._l5_duplicate_check(content_hash)
    ...
    def _l5_duplicate_check(self, content_hash: str) -> tuple[float, list]:
    ...
    def _fallback_scores(self, description: str, evidence_text: str) -> dict:
        ...
        return {
            ...
            "trigger_stuffing": max(0.10, 0.85 - len(triggers) * 0.20),
            # Hard penalty for no-evidence reports — if no file was uploaded, can't verify
            "evidence_match": 0.45 if evidence_text.strip() else 0.25,
            # Cannot verify entity consistency without AI
            "entity_consistency": 0.50,
